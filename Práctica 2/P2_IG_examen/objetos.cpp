...
_rotacion::_rotacion(vector<_vertex3f> perfil, int num) { 
    ...
    for (j=0; j<num; j++) {
  	...
  	_vertex3f rgb1(0,0.7,0);
  	_vertex3f rgb2(0.54,0.7,0.1);
    // Caras —> vamos rellenando las caras con los vertices obtenidos
    for (j=0; j<num; j++) {
        for (i=0; i<num_aux-1; i++) {
            ...
            caras.push_back(caras_aux);
      		...
      		if(j%2 == 0){
          		colores.push_back(rgb1);
          		colores.push_back(rgb1);
      		}else{
              	colores.push_back(rgb2);
              	colores.push_back(rgb2);
      		}
        }
    }
...
void _rotacion::draw_solido_caras() { 
	srand (time(NULL));
